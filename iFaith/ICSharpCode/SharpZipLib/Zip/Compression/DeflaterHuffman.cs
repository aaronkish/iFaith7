namespace ICSharpCode.SharpZipLib.Zip.Compression
{
    ...
    public class DeflaterHuffman
    {
        ...
        public bool IsFull()
        {
            return ((this.last_lit + 0x10) >= BUFSIZE);
        }
        ...
        public bool TallyDist(int dist, int len)
        {
            ...
            int num2 = this.L_code(len - 3);
            (numArray = this.literalTree.freqs)[(int) (ptr = (IntPtr) num2)] = (short) (numArray[(int) ptr] + 1);
            if ((num2 >= 0x109) && (num2 < 0x11d))
            {
                this.extra_bits += (num2 - 0x105) / 4;
            }
            int num3 = this.D_code(dist - 1);
            (numArray = this.distTree.freqs)[(int) (ptr = (IntPtr) num3)] = (short) (numArray[(int) ptr] + 1);
            if (num3 >= 4)
            {
        ...
        public class Tree
        {
            ...
            private void BuildLength(int[] childs)
            {
                ...
                for (int j = num - 1; j >= 0; j--)
                {
                    ...
                    {
                        int num8 = numArray[j];
                        this.bl_counts[(int) (ptr = (IntPtr) (num8 - 1))] = this.bl_counts[(int) ptr] + 1;
                        this.length[childs[2 * j]] = (byte) numArray[j];
                    }
                }
                if (num3 != 0)
                {
                    int num9 = this.maxLength - 1;
                    do
                    {
                        while (this.bl_counts[--num9] == 0)
                        {
                        }
                        do
                        {
                            this.bl_counts[(int) (ptr = (IntPtr) num9)] = this.bl_counts[(int) ptr] - 1;
                            this.bl_counts[(int) (ptr = (IntPtr) (++num9))] = this.bl_counts[(int) ptr] + 1;
                            num3 -= ((int) 1) << ((this.maxLength - 1) - num9);
                        }
                        while ((num3 > 0) && (num9 < (this.maxLength - 1)));
                    }
                    while (num3 > 0);
                    ...
                    {
                        int num12 = this.bl_counts[k - 1];
                        while (num12 > 0)
                        {
                            int index = 2 * childs[num10++];
                            if (childs[index + 1] == -1)
                            {
            ...
            public void CalcBLFreq(DeflaterHuffman.Tree blTree)
            {
                ...
                while (index < this.numCodes)
                {
                    ...
                    else
                    {
                        ...
                        if (num != num4)
                        {
                            (numArray = blTree.freqs)[(int) (ptr = (IntPtr) num4)] = (short) (numArray[(int) ptr] + 1);
                            num3 = 0;
                        }
                    }
                    num = num4;
                    index++;
                    while ((index < this.numCodes) && (num == this.length[index]))
                    {
                        index++;
                        if (++num3 >= num5)
                        {
                    ...
                    if (num3 < num6)
                    {
                        (numArray = blTree.freqs)[(int) (ptr = (IntPtr) num)] = (short) (numArray[(int) ptr] + ((short) num3));
                    }
                    else
                    {
                        if (num != 0)
